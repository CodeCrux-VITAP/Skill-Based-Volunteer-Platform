    ...
    match user
    {
        Some(Otp)=>{
            if Otp.otp==v.otp
            {
                ...
                ").bind(("email",v.email)).bind(("password",v.password)).await.unwrap();
                let asdf:Option<Validuser>=res.take(0).unwrap();
                match asdf
                {
                    Some(Validuser)=>{
                        let token=Getjwt(Validuser.id);
...
pub async fn Deleteotp(State(db):State<Arc<Surreal<Any>>>,Json(v):Json<User>)-> impl IntoResponse
{
    let mut res=db.query(format!("delete from otp where email='{}'",v.email)).await.unwrap();
    let result:Option<Otp>=res.take(0).unwrap();
    println!("{:?}",result);
    match result
    {
        Some(Otp)=>{StatusCode::INTERNAL_SERVER_ERROR}
        None=>{
            StatusCode::OK
        }
    }
...
pub  fn Getjwt(id:surrealdb::sql::Thing)->String
{
    let claim=Claims{
        sub:id,
        ...
        let res:Option<bool>=db.query(format!("crypto::bcrypt::compare('{}','{}')
            ...
            match res{
                Some(true)=>{
                    ...
                    (StatusCode::OK,Getjwt(Validuser.id))
